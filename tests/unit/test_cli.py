...
from inspire_classifier.cli import predict, train_classifier


def test_classifier_predict_cli_with_classifier_base_path():
    input_arguments = predict.make_context(
        "predict-coreness", args=["foo", "bar", "-b", "."]
    )
...
def test_classifier_train_cli_correctly_parses_arguments():
    input_arguments = train_classifier.make_context(
        "train", args=["-l", "15", "-c", "14", "-b", "."]
    )
